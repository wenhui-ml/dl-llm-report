...
本报告以公开论文、技术报告与官方文档为主要信息源。关键定量结论（如 DeepSeek-V3 的 671B 总参数 / 37B 激活参数 / 2.79M H800 GPU$\cdot$hours 训练成本、Qwen3-235B-A22B 的 128 专家 / 激活 8、LLaMA-2 70B 的 GQA 实验等）均引用自原始技术报告或论文。行业动态截至 2026 年 9 月 5 日：2025 年的主要事件（GPT-5/GPT-5.2、Gemini 3、Claude Opus 4.5、Qwen3、Kimi K2、Sora 2 等）与 2026 年以来的关键进展（DeepSeek-V4、Qwen3.5、Gemini 3.1、Claude Opus 5、NVIDIA 收购 Hugging Face（约 129 亿美元，2026 年 9 月 3 日官宣）、GPT-6 Astra、K2 Horizon 等）均经官方公告与多个公开报道交叉核实；个别仍在演进中的细节（GPT-6 的安全分级披露、Meta Muse Spark 的第三方评测、收购的反垄断审查进展）以最终官方披露为准，读者引用时建议回溯原始来源核实。
\end{infobox}

\begin{figure}[htbp]
\centering
\input{figs/fig01_macro_architecture}
...
\centering
\resizebox{\textwidth}{!}{%
\input{figs/fig03_architecture_timeline}
}%
...
\begin{figure}[htbp]
\centering
\input{figs/fig04_transformer_block}
...
\begin{figure}[htbp]
\centering
\input{figs/fig05_attention_pipeline}
...
\begin{figure}[htbp]
\centering
\input{figs/fig06_training_pipeline}
...
DeepSeek-V4-Pro & 1.6T & 49B & 细粒度 MoE + DSA 稀疏注意力（2026）\\
...
\textbf{Mamba 的关键创新是“选择性”（selectivity）}：早期线性 SSM（如 S4）的状态转移矩阵 $A, B, C$ 是与输入无关的常量，等价于一个固定滤波器，无法实现“看内容决定记什么”。Mamba 让 $A, B, C$ 成为当前输入 $x_t$ 的函数（选择性扫描，selective scan），并以硬件感知的并行 scan 算法实现——兼顾 RNN 的 $O(1)$ 递推推理与 GPU 上的并行训练。实际部署中的共识是\emph{混合架构}：用少量注意力层负责“精确检索”（注意力是精确查找表，SSM 是压缩记忆），如 Jamba、Zamba 以及 2025 年 NVIDIA Nemotron-H 等“注意力 + SSM 交替”设计，在长上下文成本与检索精度之间取得平衡。2026 年，这一“混合即默认”的判断被旗舰开源模型坐实：Qwen3.5 采用 Qwen3-Next 架构——Gated DeltaNet（门控增量线性注意力）与 Gated Attention 混合、叠加高稀疏度 MoE 与 MTP，397B 总参数仅激活 17B，长上下文解码吞吐最高达 Qwen3-Max 的 19 倍；DeepSeek-V4 则以 token 级压缩 + DSA 稀疏注意力把 1M 上下文做成全线标配。两者一个强化“压缩态”、一个把“逐字索引”再稀疏化，殊途同归地走向 10.3 节 CCH 所刻画的“双通道完备混合体”。
...
Gemini 3.1 Pro（2026-02） & 推理与 Agentic 深化 & ARC-AGI-2 77.1\%，约为 3 Pro 两倍\\
Qwen3.5（2026-02） & Qwen3-Next：线性注意力 + 稀疏 MoE & 397B-A17B；吞吐最高 19$\times$，201 种语言\\
DeepSeek-V4（2026-04） & token 压缩 + DSA 稀疏注意力 & 1M 上下文标配，Agentic 编码开源 SOTA\\
Llama 4 & 大规模 MoE + 长上下文 & 曾为开源旗舰（评测有争议；Llama API 已于 2026-07 下线）\\
\bottomrule
\end{tabular}
...
\begin{figure}[htbp]
\centering
\input{figs/fig08_inference_system}
...
\begin{itemize}
  ...
  \item \textbf{稀疏注意力（DSA）}：token 级压缩 + 稀疏检索式注意力，DeepSeek-V3.2 起生产化、V4 将其与 1M 上下文绑定为主打特性——把“全量 KV 精确复算”改为“压缩摘要 + 按需查账”，长上下文成本降一个数量级
\end{itemize}
...
\]
这一“第三扩展轴”（参数 $N$、数据 $D$、推理算力 $R$）是 2025--2026 年大模型竞争的主战场（详见第 10 章）。值得注意：测试时计算并非只有“生成更多 token”一种形态——让计算在潜空间循环、把深度本身变成测试时资源的“不出声思考”，是另一条平行路线（见下文循环深度一节）。

\begin{figure}[htbp]
\centering
\input{figs/fig09_testtime_compute}
...
GPT-6 Astra（2026-09） & 关键能力分级 + 前沿保障 & ARC-AGI $\approx$99\%；首触 Critical 网络安全红线\\
DeepSeek-R1 & 纯 RL + GRPO + 规则奖励 & RL 成本 29.4 万美元，开源\\
DeepSeek-V4（2026-04） & 思考/非思考双模一体 & 1M 上下文；Agentic 编码开源 SOTA\\
...
\subsection{循环深度与潜空间推理：不出声的思考}

此前讨论的测试时计算都发生在\textbf{token 空间}：模型把思考“写出来”（长 CoT、思考预算）。存在一条平行路线——让计算在\textbf{潜空间}里循环，把“网络深度”本身变成测试时资源。

\textbf{谱系与转折}：Universal Transformer（2018，层间共享权重循环展开）与 ACT/PonderNet 一系自适应计算是先驱，但长期停留在“性能不及固定深度同算力模型”的尴尬境地；转折点是 2025 年的两项工作：
\begin{itemize}
  \item \textbf{循环深度潜空间推理}（Geiping et al., 2025，arXiv:2502.05171）：架构为 \emph{Prelude--Core--Coda} 三段（图~\ref{fig:recdepth}）——Prelude 编码输入，中段循环核心按 $s_{i+1}=\mathrm{Core}(s_i, e)$ 在潜空间迭代（每步重新注入输入 $e$ 以稳定训练），Coda 读出分布。3.5B 参数、800B token 的概念验证显示：随迭代步数增加，推理基准持续提升，最高表现出约 50B 参数模型的计算负载；相邻两步输出分布的 KL 收敛可作\emph{零样本提前退出}判据——不同 token 自然分配到不同深度。与深度均衡模型（DEQ）的区别在训练目标：用截断 BPTT 展开循环，而非对不动点求导，因此不要求算子收敛（实践中多数 token 平滑收敛，少数走“轨道”式迭代同样有用）。
  \item \textbf{Mixture-of-Recursions}（Google DeepMind, 2025）：把“每个 token 递归几步”交给路由器学习（expert-choice / token-choice 两种分配粒度），配合跨递归步的 KV 共享策略，在同等训练算力下取得优于参数量匹配基线的每 FLOP 性能——递归深度第一次在旗舰级工程里“回本”。
\end{itemize}

\textbf{为什么这条路线突然重要}——用 10.3 节的语言说：固定深度的 log-精度 Transformer 被约束在 $\mathrm{TC}^0$ 类内（“电路墙”），串行组合深度买不来。CoT 是\emph{用 token 长度购买深度}，循环深度是\emph{用参数复用购买深度}——两者都把“平面计算图”改造成“任意深度计算图”，只是付费货币不同。循环深度的独特优势：思考不占用上下文窗口、不留可被蒸馏的可见思维链（直接消除“只给思考摘要”式的安全暴露面）、对难以言表的推理（空间/符号操作）更友好；代价是可解释性更差、跨步 KV 管理（共享 vs 独立）与延迟调度更复杂，且迄今公开验证的规模（10B 级）仍落后主流推理模型。

\begin{figure}[htbp]
\centering
\resizebox{0.97\textwidth}{!}{%
\input{figs/fig11_recurrent_depth}
}%
\caption{循环深度 Transformer（潜空间推理）示意：Prelude 编码后，循环核心按需迭代 $r$ 步（测试时可调），Coda 读出分布；KL 收敛判据允许逐 token 提前退出。与 CoT“用 token 长度买深度”不同，循环深度“用参数复用买深度”，思考不占用上下文窗口（Geiping et al., 2025；DeepMind MoR, 2025）。}
\label{fig:recdepth}
\end{figure}

2026 年，该方向已从单点论文扩展为一个小型生态：Looped Language Model 的规模化训练、连续潜空间推理训练、Recursive Language Models、Inner Thinking Transformer 等变体相继出现。它是否已进入 GPT-6 / o 系旗舰的生产实现，各实验室均未披露——业界共识是“快慢路由 + 潜空间计算”的某种混合是下一步，但公开证据仍属间接。
...
\centering
\resizebox{\textwidth}{!}{%
\input{figs/fig12_multimodal_architecture}
}%
...
Gemini 3/3.1 & 原生多模态 + Deep Think & 文本+图+音+视频统一\\
\bottomrule
\end{tabular}
\caption{代表性 VLM}
\end{table}
...
\centering
\resizebox{\textwidth}{!}{%
\input{figs/fig13_defense_in_depth}
}%
...
\begin{itemize}
  ...
  \item \textbf{Agent 失控进入现实账本（2026）}：7 月 Anthropic 披露三起 Claude 模型未经授权访问真实计算机系统的事件，并邀请 METR 独立审查；几乎同期，OpenAI 披露 GPT-6 Astra 首次触发其准备度框架的“Critical”网络安全能力红线，同步给出前沿保障（frontier safeguards）与 10 亿美元网络防御承诺；Google 则把“主动网络防御”产品化（Gemini 3.8 Flash Cyber）——前沿模型的“能力--风险”非对称从推演题变成了部署决策题。
\end{itemize}
...
\begin{itemize}
  \item \textbf{EU AI Act}（2024 年 8 月生效，2025--2027 分阶段实施）：\emph{风险分级}监管——不可接受风险（如社会评分）禁止、高风险（招聘、医疗）严格合规义务（数据治理、日志、人工监督）、有限风险（透明度义务）、最小风险自由。全球首个综合性 AI 法，具有“布鲁塞尔效应”——2025 年 8 月起 GPAI（通用模型）义务生效；2026 年 8 月起附件 III 高风险系统义务与第 50 条透明度义务全面适用，EU AI Office 执法团队同步扩编，进入实质执法阶段。
...
\centering
\resizebox{0.97\textwidth}{!}{%
\input{figs/fig15_cch_access_structure}
}%
...
这张表也解释了 2025--2026 年架构竞争的真实格局：\textbf{纯 SSM、纯注意力、纯检索都不是终点，“混合 + 外置”才是通往访问完备的工程路线}——Jamba 类混合架构、长上下文与 RAG 并用的生产组合、把推理深度转移到测试时的推理模型（第 7 章），都是在同一本“访问结构账”上的不同记账方式。事实上，2026 年的旗舰开源模型已经按这一蓝图收敛：Qwen3.5 的 Gated DeltaNet 线性注意力（压缩态）+ 稀疏注意力 MoE（逐字索引），与 DeepSeek-V4 的 token 压缩 + DSA（1M 上下文标配）不约而同走向“双通道混合体”——CCH 的“访问完备类”正在从理论类型变成架构竞争的显性终点。最后需要强调边界：CCH 的实验仍是小规模预注册测试，“领域级收敛”是猜想而非定理，且其结论限于无 CoT 的固定预算情形——但“能力由访问结构定价”这一视角，对理解第 11 章的算力--资本--生态格局与第 12 章的趋势判断，已经足够有力。

\subsubsection{案例：GLM-5.3 的“后训练跃迁”能否用访问结构解释}

2026 年 8 月，智谱 GLM-5.3 提供了一个值得用 CCH 语言拆解的现象：官方口径下\textbf{基座不变}，仅靠“后训练 scaling”——数十倍的长程任务环境、更多样的环境类型、超长的强化学习时长——就把编程与 agentic 能力较 GLM-5.2 推高约 50\%，成为编程最强开源模型，并以更低成本达到与 GPT-5.6 同级的智能；其小杯 \textbf{GLM-5.3-Flash}（320B 总参/18B 激活，GLM-5 系首个原生多模态）以旗舰 1/20 的定价，被第三方评测与厂商口径称为比肩 GPT-5.6 / Claude Opus 4.8 级旗舰——而后者据估计总参数大一个数量级以上。

\textbf{CCH 能解释的部分}：CCH 的定价对象是\emph{能力类资格}，而非参数规模——表征由预训练收敛“免费赠予”（PRH），能力由访问结构“购买”。GLM-5.3 的后训练配方恰好在“购买访问结构的使用熟练度”：长程任务环境训练的是跨长上下文\emph{维持运行状态}（压缩态通道的可靠性），agentic 环境训练的是工具与检索结果的\emph{逐字回查}（外置逐字索引）与多步历史的工作集组装。当表征层已经收敛，“每 token 所需的激活计算”由查询类决定、而非由知识总量决定——知识躺在权重里，查询执行发生在访问结构上。于是 18B 激活 + 完备双通道即可获得\emph{同一能力类的成员资格}；数 T 参数是为长尾稳健性支付的溢价，那是\textbf{类内价格差而非类别差}。这与 CCH“能力收敛到访问完备混合体这一类、类内差异按墙付费”的图景一致，也解释了配方为何可复制：Qwen3.5 与 DeepSeek-V4 同期落入同一类（见上），GLM-5.3 并非孤例。

\textbf{CCH 不能解释的部分（必须并置的三个因素）}：其一，\emph{蒸馏链}——GLM 系后训练环境大概率包含强模型生成的轨迹，等于经由数据通道“借用了”教师级能力，这在 CCH 的架构通道之外，也决定了跃迁的绝对高度被教师水平锁定（第 7 章蒸馏边界的再现）；其二，\emph{评测口径}——agentic/编程跑分依赖 harness（ZCode、OpenCode、Claude Code 等）与外部工具，“访问结构”部分由模型之外承载，且 10.1 节的基准污染与度量伪影警告在此同样适用；其三，\emph{CCH 自身的边界}——定理域限于无 CoT 的固定预算情形，“领域级收敛”仍是猜想。因此恰当的表述是：GLM-5.3 现象\emph{与 CCH 图景一致}，是“表征收敛免费、能力按访问结构定价”的一个支持性案例而非定理级证明；它更提示我们，2026 年“后训练跃迁”的三要素是\emph{环境规模 $\times$ 蒸馏质量 $\times$ 训练时长}——参数规模已退居第二序，而“访问结构的熟练使用”成为后训练真正在练的东西。
...
本章汇总 2025 年至 2026 年 9 月初的行业关键进展。\textbf{阅读提示}：本章内容主要来自厂商官方公告与行业公开报道，模型发布、收购官宣等主要事件均已交叉核实；仍在演进中的细节（反垄断审查结果、最新模型的第三方评测与复现）以最终官方披露为准；引用这些数据用于正式文档前，建议回溯一手来源核实。

\textbf{本章三条主线}：(1)\textbf{推理与 Agent 成为产品主形态}——o1$\to$o3$\to$GPT-6、R1$\to$V4、Gemini 3.1/3.8 系列把“思考预算”做成产品开关，Operator/Computer Use 把“执行动作”做成标准能力，长时程 Agent 任务成为新的必争基准；(2)\textbf{开源追平并在局部反超闭源}——Qwen3.5、DeepSeek-V4、K2 Horizon 在核心基准上比肩甚至超越同期闭源旗舰，Meta 则于 2026 年 7 月下线 Llama API、转向“开源 + 闭源”双轨；(3)\textbf{算力整合与关键能力治理并进}——NVIDIA 收购 Hugging Face 完成生态纵向整合、头部云厂商年资本开支迈向 5000 亿美元量级；GPT-6 首次触发“Critical”网络安全能力红线、EU AI Act 高风险义务于 2026 年 8 月全面适用，“能力分级 + 部署门槛”成为治理新范式。
...
\midrule
OpenAI & o3/o4-mini（2025-04）、GPT-5（08）$\to$ 5.2（12）$\to$ 5.6（2026 上半年）$\to$ \textbf{GPT-6 Astra}（2026-09）、Sora 2、Operator & 推理 + Agent + 多模态统一；GPT-6 首触 Critical 网络红线，承诺 \$1B 网络防御\\
Google & Gemini 3 Pro（2025-11，LMArena 1501）$\to$ 3.1 Pro（2026-02，ARC-AGI-2 77.1\%）$\to$ 3.7（I/O 2026）$\to$ 3.8 Flash Cyber（2026-08）、Veo 3.1、Genie 3 & 原生多模态 + 世界模型 + 主动网络防御\\
Anthropic & Claude 4（05）$\to$ Opus 4.5（11，SWE-bench 80.9\%）$\to$ Fable 5 / Mythos 5（2026 上半年）$\to$ \textbf{Opus 5}（2026-07） & 编程与 Agent 标杆；Fable 5 第三方评测 SWE-bench 约 95\%，Opus 5 同价升级\\
Meta & Llama 4（2025-04）$\to$ 下线 Llama API、转“开源+闭源”双轨（2026-07）$\to$ 新旗舰 Muse Spark（2026-09 报道） & Llama 时代收官；报道称 Muse Spark 逼平 Fable 5\\
xAI & Grok 3（02）$\to$ Grok 4（07，多智能体）$\to$ Grok 4.6（2026-08，长时程 Agent、超低价） & 10 万卡 Colossus；Grok 5 训练中（Colossus 2）\\
DeepSeek & R1（01）$\to$ V3.2（2025-12，DSA）$\to$ \textbf{V4}（2026-04：Pro 1.6T-A49B / Flash 284B-A13B）$\to$ V4-Pro GA（08-13） & MLA、纯 RL 推理；1M 上下文标配，Agentic 编码开源 SOTA\\
阿里 & Qwen3（04）$\to$ Qwen3-Coder（07）$\to$ \textbf{Qwen3.5}（2026-02，397B-A17B）$\to$ 3.5-Max 预览（03） & Qwen3-Next 混合架构；201 种语言；吞吐最高 19$\times$；开源矩阵最全\\
月之暗面 & Kimi K2（2025-07）$\to$ K2.6 / K2.7 Code（Copilot 首个开源模型）$\to$ \textbf{K3}（2026-07 开源，kimi-k3-max Arena 综合第 10） & K3 引发“减速主义”之争；2026-09 秘密交表港交所（Pre-IPO 估值 500 亿美元）\\
智谱（Z.AI） & GLM-4.5（07）、2026-01 港股上市 $\to$ \textbf{GLM-5.3}（2026-08：基座不变 + 后训练 scaling，编程最强开源）$\to$ GLM-5.3-Flash（320B-A18B 原生多模态，价格 1/20） & 大模型第一股；ARR 半年 15 倍至 \$1B；全栈国产算力；10.3 节有专节解读\\
NVIDIA & B200/GB200、约 \$12.9B 收购 Hugging Face（\textbf{2026-09-03} 官宣） & 算力 + 生态纵向整合（HSR 审查中）\\
IFM（MBZUAI） & \textbf{K2 Horizon}（2026-09）：0.9B--375B 六模型 & 全流程开源（数据/检查点/日志），小尺寸三档 SOTA；首创 MoVA\\
\bottomrule
\end{tabular}
...
\textbf{GPT-6 Astra 为什么值得单独一记}：它把本章三条主线同时推到了新位置。\emph{能力上}，公开报道口径的 ARC-AGI 接近满分（约 99\%）、数学与攻防类测试近乎满分——距 GPT-5.2 / 5.6 仅约 9 个月的代际跨越。\emph{治理上}，它是首个触发 OpenAI 准备度框架“Critical”级（网络安全域）的模型，配套发布《Path to Astra：关键能力与前沿保障》，同步给出能力分级、部署门槛、红队强度与 10 亿美元网络防御承诺——第 9 章的“能力--风险非对称”与 10.3 节的“电路墙”在产品层面首次正面相遇。\emph{市场上}，“AGI 时代开端”的宣称并未推高算力叙事，官宣次日 NVIDIA 股价不涨反跌（约 $-2\%$）——市场开始按“效率曲线”而非“堆卡曲线”给基础设施定价，与下文算力一节的成本锚点判断一致。技术实现层面 OpenAI 披露有限：业界普遍推断其延续“统一路由快慢思考 + agentic RL + 大规模 MoE”路线，是否引入潜空间/循环计算（见第 7 章末节）尚无公开证据——把“基准分数的代际跳变”直接归因于“架构革命”，目前仍缺乏支撑。
...
\begin{itemize}
  ...
  \item \textbf{10--12 月}：Claude Opus 4.5（SWE-bench 80.9\%，首个破 80）；\textbf{Gemini 3 Pro}（LMArena 首破 1500 Elo，GPQA 91.9\%）；Veo 3.1；GPT-5.1 / 5.2；DeepSeek-V3.2（DSA 稀疏注意力生产化）；据报道 Manus 被 Meta 收购；据 Wired 报道，Gemini 3 的发布一度触发 OpenAI 内部 Code Red；
  \item \textbf{2026 年 1 月}：智谱 AI 港股上市（全球大模型第一股）、MiniMax 上市首日涨 109\%；LeCun 离开 Meta 创立 AMI Labs 押注世界模型；三大实验室同步进军医疗 AI。
  \item \textbf{2026 年 2 月}：Google 发布 Gemini 3.1 Pro（ARC-AGI-2 77.1\%，约为 3 Pro 两倍）；阿里除夕夜开源 \textbf{Qwen3.5}（397B-A17B，Qwen3-Next 混合架构，201 种语言），3 月再推 Qwen3.5-Max 预览版；
  \item \textbf{2026 年 4 月}：DeepSeek-\textbf{V4} 开源预览（Pro 1.6T/激活 49B、Flash 284B/激活 13B，1M 上下文成为全线标配）；
  \item \textbf{2026 年 5 月}：Google I/O 宣布“Agentic Gemini 时代”（Gemini 3.7）；Anthropic 于上半年推出新一代前沿系列 Fable 5 与 Mythos 5；
  \item \textbf{2026 年 7 月}：\textbf{Claude Opus 5} 发布（每百万 token \$5/\$25 同价大升级，effort 档位可调）；Meta 宣布下线 Llama API、转向双轨；Anthropic 披露三起 Claude 未授权访问系统事件并邀 METR 独立审查；月之暗面开源 \textbf{Kimi K3}（后升至 Arena 综合前十；OpenAI 战略主管批其开源为“减速主义”，总裁 Brockman 回应“K3 确实不错”）；K2.6 因 DoorDash 采用而遭美国会两委员会调查——中国开源模型首次深度进入美国生产链；
  \item \textbf{2026 年 8 月}：DeepSeek-V4-Pro 正式版 GA（合并推理/对话双模）；xAI Grok 4.6（长时程 Agent、价格下探）；Gemini 3.8 Flash Cyber 把“主动网络防御”做成产品；EU AI Act 高风险义务 8 月 2 日全面适用；智谱 \textbf{GLM-5.3}（基座不变 + 后训练 scaling：编程最强开源、较 5.2 +50%，权重 8 月底开源）与 \textbf{GLM-5.3-Flash}（320B-A18B 原生多模态、价格 1/20、全国产算力）相继发布；
  \item \textbf{2026 年 9 月初}：NVIDIA 官宣约 129 亿美元收购 Hugging Face（9 月 3 日）；IFM（MBZUAI）发布 K2 Horizon 六模型全流程开源全家桶；OpenAI 发布 \textbf{GPT-6 Astra}（ARC-AGI 约 99\%，称“AGI 时代开端”，同时首次触发 Critical 网络安全红线、承诺 10 亿美元网络防御）；Meta 新旗舰 Muse Spark 曝出逼平 Fable 5；腾讯混元 Hy4 Preview 开源（770B-A49B、1M 上下文）；9 月 3 日海外主流 AI 服务集体宕机约 3.5 小时、国产服务未受影响。
\end{itemize}
...
\begin{itemize}
  \item \textbf{DeepSeek}：R1（RL 成本 29.4 万美元）$\to$ V3.1 / V3.2（混合思考、强代码、DSA 稀疏注意力）$\to$ \textbf{V4}（2026-04：Pro 1.6T/激活 49B、1M 上下文标配；8 月 V4-Pro 正式版）；以 MLA / MTP / FP8 / 无辅助损失 MoE / 稀疏注意力的架构-系统协同路线持续压低训练与推理成本，是两年间行业成本曲线的锚点；
  \item \textbf{阿里 Qwen}：Qwen3 全系开源（0.6B--235B，Apache 2.0，36T token、119 种语言）$\to$ \textbf{Qwen3.5}（2026-02：Gated DeltaNet 线性注意力 + 稀疏 MoE 的 Qwen3-Next 架构，397B-A17B、201 种语言、吞吐最高 19$\times$）与 Qwen3.5-Max 预览版；衍生模型生态为全球最大；
  \item \textbf{月之暗面 Kimi}：K2 以 1T 级 MoE 全量开源并登顶开源榜，验证 Agentic 数据工程 + 大 MoE 路线 $\to$ K3（2026-07 开源，Arena 综合前十），K2.6 被 DoorDash 采用引发美国会调查——中国开源模型首次深度进入美国生产链；已启动港股 IPO（Pre-IPO 估值 500 亿美元）；
  \item \textbf{智谱 GLM}：GLM-4.5（355B MoE）开源，2026 年 1 月率先登陆港交所成为全球大模型第一股 $\to$ \textbf{GLM-5.3}（基座不变 + 后训练 scaling，编程最强开源、较 5.2 +50%）与 GLM-5.3-Flash（320B-A18B 原生多模态、价格 1/20、全栈国产算力），ARR 半年增长 15 倍至 10 亿美元、1GW 国产芯片数据中心投运，公司更名 Z.AI（案例解读见 10.3 节）；
  \item \textbf{腾讯混元}：Hy4 Preview 开源（2026-09：770B-A49B、1M 上下文），官方称“预训练与后训练并进”，重返开源第一梯队；
...
\textbf{算力}：NVIDIA Blackwell（B200/GB200）进入规模部署，\textbf{2026 年 9 月 3 日}官宣以约 \textbf{129 亿美元}收购 Hugging Face（含约 119 亿美元现金，进入 HSR 反垄断审查）——芯片 + 框架 + 模型生态的纵向整合引发开源社区对中立性的担忧；云厂商自研芯片（TPU、Trainium、MTIA）继续分流，2026 年头部云厂商 AI 资本开支预计合计突破 5000 亿美元，吉瓦级数据中心的能源与环评（燃气发电、核电 PPA）成为新的瓶颈议题。\textbf{资本}：OpenAI 完成 400 亿美元融资（估值 3000 亿美元）并于 2025 年 10 月完成公益公司（PBC）重组，公开报道口径的累计算力合同承诺已超万亿美元、2026 年营收目标 300 亿美元；Anthropic F 轮 130 亿美元（估值 1830 亿美元）；行业在 AI 泡沫争议中进入以真实 ROI 论成败的价值证明之年。\textbf{开源生态}：MCP 协议捐赠开源基金会、成为 Agent 连接工具的事实标准；OpenAI 重回开源（GPT-oss）、Llama 4 争议与 Llama API 下线动摇 Meta 的开源旗手地位、Qwen/Kimi/GLM 补位，IFM（MBZUAI）以“全流程开源”的 K2 Horizon 加码——开源重心从单点旗舰转向\emph{全栈生态}（权重 + 数据配方 + 推理引擎 + Agent 框架 + 协议）。
...
\begin{figure}[htbp]
\centering
\input{figs/fig16_data_flywheel}
...
\begin{enumerate}
  ...
  \item \textbf{开源东移与路线分化}：Qwen、DeepSeek、Kimi、GLM 领跑并覆盖生产级推理与 Agent 能力；Meta 2026-07 下线 Llama API 转向双轨，IFM 等新玩家以“全流程开源”（数据配方 + 检查点 + 日志）加码
  \item \textbf{端侧 AI}：隐私优先的本地推理（1B--10B 模型进手机/PC）
  \item \textbf{世界模型}：从语言走向物理（Genie 3、AMI Labs），为具身智能铺路
\end{enumerate}

2026 年值得盯住的四个变量：一是\emph{世界模型}能否从演示走向可用（Genie 3 类可交互世界、AMI Labs 路线）；二是\emph{上市潮}能否经受盈利检验（头部的 IPO 预期与港交所已上市公司的商业化数据）；三是\emph{监管全面生效}（EU AI Act 2026 年 8 月高风险义务全面适用、中国标识义务常态化）对产品形态的实际约束；四是\emph{关键能力治理}的走向——GPT-6 Astra 被称为“AGI 时代开端”的同时首次触发 Critical 网络安全红线，前沿模型的部署门槛正从“基准分数”转向“能力分级 + 红线管理”。
...
DSA & DeepSeek Sparse Attention & 稀疏注意力：token 压缩 + 按需查账（V3.2 起生产化）\\
Gated DeltaNet & --- & 门控增量线性注意力（Qwen3-Next 的压缩态通道）\\
...
循环深度 & Recurrent Depth & 测试时迭代中段块，深度成为推理资源\\
潜空间推理 & Latent Reasoning & 在连续隐状态中思考，不生成思考 token\\
...
\begin{itemize}
  ...
  \item Geiping et al., 2025. \emph{Scaling up Test-Time Compute with Latent Reasoning: A Recurrent Depth Approach}（arXiv:2502.05171）——循环深度潜空间推理（Prelude--Core--Coda）；Google DeepMind, 2025. \emph{Mixture-of-Recursions}（技术报告）——动态递归深度与 KV 共享；
  \item Dehghani et al., 2018. \emph{Universal Transformers}（arXiv:1807.03819）；Graves, 2016. \emph{Adaptive Computation Time}（arXiv:1603.08983）——循环深度/自适应计算的思想源头；
...
\textbf{2026 年前沿动态溯源}（官方公告为主）
\begin{itemize}
  \item OpenAI, 2026. \emph{GPT-6 Astra}（系统卡与 frontier safeguards 公告，2026-09）；
  \item Anthropic, 2026. \emph{Claude Opus 5 / Fable 5 / Mythos 5} 模型公告（2026-07）；2026-07-30 未授权访问事件的披露与 METR 独立审查说明；
  \item Google, 2026. \emph{Gemini 3.1 Pro}（2026-02）与 \emph{Gemini 3.8 Flash Cyber}（2026-08）官方博客；
  \item DeepSeek-AI, 2026. \emph{DeepSeek-V4 技术报告}（Hugging Face：deepseek-ai/DeepSeek-V4，DSA 与 1M 上下文；V4-Pro GA 2026-08-13）；
  \item Qwen Team, 2026. \emph{Qwen3.5} 技术博客（Qwen3-Next 混合架构，2026-02）；
  \item IFM / MBZUAI, 2026. \emph{K2 Horizon} 全流程开源发布公告（Apache 2.0，2026-09）；
  \item Zhipu / Z.AI, 2026. \emph{GLM-5.3 / GLM-5.3-Flash} 技术博客与开源公告（2026-08，“基座不变 + 后训练 scaling”）；Moonshot AI, 2026. \emph{Kimi K3} 开源公告（2026-07）；
  \item Tencent, 2026. \emph{Hunyuan Hy4 Preview} 开源公告（2026-09，770B-A49B、1M 上下文）。
\end{itemize}
